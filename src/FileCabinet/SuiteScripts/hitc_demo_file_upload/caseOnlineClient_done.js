...
define(["require", "exports"], function (require, exports) {
        ...
        jQuery(imageInputId).on('change', (event) => {
            ...
            if (input.files.length > 0) { // Else maybe they cleared the field
                ...
                reader.onload = function () {
                    ...
                    jQuery.ajax({ url: suiteletURL, data: { content: base64Str, name: `${Date.now()} ${fileName}`, fileType }, method: 'POST' }).then((response) => {
    ...
    function getFileType(fileObj) {
        ...
        if (fileObj.type == 'application/pdf')
            ...
            return 'jpg';
        else if (fileObj.name.includes('.doc'))
            return 'doc';
        else if (fileObj.name.includes('.txt'))
            return 'txt';
        else
            return 'bin';
    }
